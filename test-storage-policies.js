...
async function testStoragePolicies() {
  ...
  try {
    ...
    if (adminError) {
      ...
      const communityBucket = adminBuckets.find(b => b.name === 'community-images');
      if (communityBucket) {
        ...
        if (adminListError) {
          ...
          console.log(`✅ Admin can list files: ${adminFiles.length} files found`);
        }
      }
    }
    ...
    if (anonError) {
      ...
      console.log('   👉 This might be expected - anon users may not list buckets');
    } else {
    ...
    if (anonListError) {
      ...
      if (anonListError.message.includes('policies') || anonListError.message.includes('permission')) {
        console.log('   👉 This suggests policies might need to be configured for bucket listing');
      }
    } else {
    ...
    try {
      ...
      if (uploadError) {
        ...
        if (uploadError.message.includes('authenticated')) {
          console.log('   ✅ Policy working: Only authenticated users can upload');
        }
      } else {
        console.log('⚠️ Unexpected: Anon upload succeeded (this might be a policy issue)');
      }
    } catch (err) {
    ...
    if (adminBuckets && adminBuckets.find(b => b.name === 'community-images')) {
