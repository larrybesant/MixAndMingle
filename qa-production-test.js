...
const config = {
  ...
  supabaseUrl: process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://ywfjmsbyksehjgwalqum.supabase.co',
  supabaseKey: process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl3Zmptc2J5a3NlaGpnd2FscXVtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDczMzIwNjgsImV4cCI6MjA2MjkwODA2OH0.fXx5d7iRXgpJDB_jAKgtRa2pVoAPBHU9Rly0T14HsVs'
};
...
async function testEnvironmentVariables() {
  ...
  try {
    ...
    if (config.supabaseUrl.includes('localhost') || config.appUrl.includes('localhost')) {
...
async function testDatabaseSchema() {
  ...
  try {
    ...
    const tables = ['profiles', 'communities', 'community_members', 'community_posts'];
...
async function testCommunitiesFeature() {
  ...
  try {
    ...
    if (communitiesError) {
      ...
      logTest('Communities Listing', 'PASS', `Found ${communities.length} communities`);
    }
...
async function testImageUploadSystem() {
  ...
  try {
    ...
    logTest('Upload Policies', 'PASS', 'Policies configured for authenticated users');
...
async function generateQAReport() {
  ...
  if (successRate >= 90) {
    ...
    console.log('⚠️ MOSTLY READY - Some issues need attention before full production.');
  } else {
