...
async function testCompleteStorageFlow() {
  ...
  try {
    ...
    if (listError) {
      ...
      console.log(`✅ Can access bucket: ${files.length} files currently stored`);
    }
    ...
    if (anonUploadError) {
      ...
      if (anonUploadError.message.includes('policy') || anonUploadError.message.includes('security')) {
        console.log('   👉 Policy working: Authentication required for uploads');
      }
    } else {
    ...
    testUrls.forEach(testUrl => {
      ...
      console.log(`   Sample path: ${testUrl} → ${urlData.publicUrl ? 'URL generated' : 'Failed'}`);
    });
