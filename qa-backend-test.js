...
class QATestSuite {
    ...
    await this.test('Communities Schema', async () => {
      ...
      if (error) {
        if (error.message.includes('relation "communities" does not exist')) {
          throw new Error('Communities schema not set up - Run: POST /api/admin/setup-communities-schema');
        }
  ...
  async testCommunityMembersSchema() {
    await this.test('Community Members Schema', async () => {
      ...
      if (error && error.message.includes('relation "community_members" does not exist')) {
  ...
  async testCommunityPostsSchema() {
    await this.test('Community Posts Schema', async () => {
      ...
      if (error && error.message.includes('relation "community_posts" does not exist')) {
  ...
  async testStorageBuckets() {
    await this.test('Storage Buckets', async () => {
      ...
      const communityBucket = data.find(bucket => bucket.name === 'community-images');
      if (!communityBucket) {
        console.log('⚠️ Community images bucket not found - may need manual setup in Supabase');
      }
    });
  }
  ...
  async testNextJSBuild() {
    await this.test('Next.js Application Build', async () => {
      ...
      if (!fs.existsSync(manifestPath)) {
        throw new Error('Build manifest missing - application may not be properly built');
      }
    });
  }
    ...
    await this.test('Local Development Server', async () => {
      try {
        ...
        if (error.message.includes('ECONNREFUSED') || error.message.includes('fetch')) {
          throw new Error('Local development server not running - run: npm run dev');
        }
        throw error;
      }
    });
  ...
  async testTypeScript() {
    await this.test('TypeScript Configuration', async () => {
      ...
      const hasTypeScriptFiles = fs.readdirSync('app', { recursive: true })
  ...
  async runAllTests() {
    ...
    if (this.testResults.failed > 0) {
      ...
      this.testResults.details
        ...
        .forEach(test => {
          ...
          console.log(`   Solution: ${this.getSolution(test.name, test.error)}\n`);
        });
    }
    ...
    console.log('\n📋 NEXT STEPS:');
    if (this.testResults.details.some(t => t.error && t.error.includes('Communities schema not set up'))) {
    ...
    if (this.testResults.details.some(t => t.error && t.error.includes('not running'))) {
