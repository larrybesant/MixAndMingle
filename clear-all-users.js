  ...
  try {
    ...
    if (profiles && profiles.length > 0) {
      profiles.forEach(profile => {
        console.log(`- ${profile.username || 'No username'} (${profile.email || profile.id})`)
      })
    }
    ...
    if (authError) {
      ...
      console.log('💡 You may need to manually delete auth users from Supabase dashboard')
    } else {
    ...
    console.log(`- Auth users remaining: ${remainingAuthUsers?.users?.length || 0}`)
    
    if ((remainingProfiles?.length || 0) === 0 && (remainingAuthUsers?.users?.length || 0) === 0) {
      console.log('\n🎉 All users successfully cleared!')
      console.log('✨ Ready to test first account creation with language selection')
    } else {
