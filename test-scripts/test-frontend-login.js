...
async function testFrontendLogin() {
  ...
  try {
    ...
    // Wrong password
    const wrongPasswordResponse = await fetch(`${supabaseUrl}/auth/v1/token?grant_type=password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${supabaseKey}`,
    ...
    console.log('🔑 Wrong password test:', {
      ...
      error: wrongPasswordData.error?.message || wrongPasswordData.error_description
    });
    
    // Non-existent user
    const noUserResponse = await fetch(`${supabaseUrl}/auth/v1/token?grant_type=password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${supabaseKey}`,
