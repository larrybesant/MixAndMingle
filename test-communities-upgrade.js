...
async function testCommunityFeatures() {
  ...
  try {
    ...
    if (error) {
      console.log('❌ Database not set up. Please run /api/admin/setup-communities-schema');
      return;
    }
  ...
  try {
    ...
    if (communityBucket) {
      ...
      console.log('⚠️ Community images bucket not found - may need manual setup');
    }
  } catch (error) {
  ...
  console.log('✅ Community creation payload validated:', {
    ...sampleCommunity,
    avatar_url: sampleCommunity.avatar_url ? '✅ Avatar URL supported' : '❌ No avatar',
    banner_url: sampleCommunity.banner_url ? '✅ Banner URL supported' : '❌ No banner'
  });
