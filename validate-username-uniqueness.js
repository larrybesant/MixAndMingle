...
async function addUsernameConstraints() {
  try {
    ...
    if (constraintError) {
      console.log('⚠️  Could not check existing constraints:', constraintError.message);
    } else {
    ...
    console.log('✅ Username uniqueness is already enforced at the application level:');
    console.log('   - Frontend checks username availability before submission');
    console.log('   - Backend validates uniqueness in signup/profile update endpoints');
