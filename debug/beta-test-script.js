  ...
  criticalFiles.forEach((file) => {
    if (fs.existsSync(file)) {
      ...
      logTest(
        ...
        consoleLogs === 0 ? "PASS" : "WARN",
        consoleLogs > 0 ? `Found ${consoleLogs} console.log statements` : "",
      );

      // Check for proper TypeScript usage
      const hasTypeErrors =
        content.includes("any") && !content.includes("// @ts-ignore");
      logTest(
        "Code Quality",
        `TypeScript usage in ${file}`,
        !hasTypeErrors ? "PASS" : "WARN",
        hasTypeErrors
          ? 'Contains "any" types without proper justification'
          : "",
      );
    }
  });
...
function testUIComponents() {
  ...
  componentsToCheck.forEach((component) => {
    if (fs.existsSync(component)) {
      ...
      logTest(
        "UI Components",
        `Proper exports in ${component}`,
        hasDefaultExport ? "PASS" : "FAIL",
      );
...
function testAuthenticationLogic() {
  const authFiles = ["app/login/page.tsx", "lib/supabase/client.ts"];
  ...
  authFiles.forEach((file) => {
    if (fs.existsSync(file)) {
      ...
      if (file.includes("login")) {
        const hasOAuth =
          content.includes("signInWithOAuth") || content.includes("google");
        logTest(
          "Authentication",
          `OAuth implementation in ${file}`,
          hasOAuth ? "PASS" : "FAIL",
        );
      }

      // Check for session management
      const hasSessionMgmt =
        content.includes("getSession") || content.includes("getUser");
      logTest(
        "Authentication",
        `Session management in ${file}`,
        hasSessionMgmt ? "PASS" : "WARN",
      );
    }
  });
}
...
function testDatabaseIntegration() {
  ...
  if (fs.existsSync("lib/supabase/client.ts")) {
    ...
    const hasProperConfig =
      content.includes("createClient") && content.includes("supabaseUrl");
    logTest(
      "Database",
      "Supabase client configuration",
      hasProperConfig ? "PASS" : "FAIL",
    );

    const hasErrorValidation = content.includes(
      "Missing Supabase environment variables",
    );
    logTest(
      "Database",
      "Environment validation",
      hasErrorValidation ? "PASS" : "WARN",
    );
  }
  ...
  const schemaFiles = fs
    .readdirSync(".")
    .filter((f) => f.includes("schema") || f.includes("supabase"));
  logTest(
    "Database",
    "Schema files present",
    schemaFiles.length > 0 ? "PASS" : "WARN",
...
function testRealtimeFeatures() {
  ...
  pagesWithRealtime.forEach((file) => {
    if (fs.existsSync(file)) {
      ...
      const hasInterval =
        content.includes("setInterval") || content.includes("useInterval");
      logTest(
        "Realtime",
        `Real-time updates in ${file}`,
        hasInterval ? "PASS" : "WARN",
      );

      const hasCleanup =
        content.includes("clearInterval") || content.includes("return () =>");
      logTest(
        "Realtime",
        `Cleanup logic in ${file}`,
...
function testMobileResponsiveness() {
  ...
  pageFiles.forEach((file) => {
    if (fs.existsSync(file)) {
      ...
      logTest(
        "Mobile",
        `Responsive design in ${file}`,
        hasTailwindResponsive ? "PASS" : "WARN",
      );

      const hasFlexGrid = content.includes("flex") || content.includes("grid");
      logTest(
        "Mobile",
        `Modern layout in ${file}`,
        hasFlexGrid ? "PASS" : "WARN",
      );
    }
  });
}
...
if (testResults.issues.length > 0) {
  ...
  testResults.issues.forEach((issue, index) => {
    console.log(
      `${index + 1}. [${issue.category}] ${issue.test}: ${issue.details || "See details above"}`,
    );
  });
}
...
const severity =
  testResults.failed > 0
    ? "HIGH"
    : testResults.issues.filter((i) => i.status === "WARN").length > 5
      ? "MEDIUM"
      : "LOW";
...
if (severity === "HIGH") {
  console.log(
    "❗ RECOMMENDATION: Fix critical issues before proceeding to beta",
  );
  process.exit(1);
  ...
  console.log(
    "🎉 RECOMMENDATION: Ready for beta testing with manual verification",
  );
}
