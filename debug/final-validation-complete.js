...
async function validateCompleteFlow() {
  ...
  try {
    ...
    const testImageFile = new File([testImageBlob], "test-avatar.png", {
      type: "image/png",
    });
...
async function testPhotoUploadOnly() {
  ...
  try {
    ...
    if (!user) {
      console.log(
        "⚠️ No user logged in. Please log in first or run full validation.",
      );
      return;
    }
    ...
    canvas.toBlob(async (blob) => {
      const testFile = new File([blob], "test-photo.png", {
        type: "image/png",
      });
...
console.log(
  "• testPhotoUploadOnly() - Test photo upload only (requires login)",
);
