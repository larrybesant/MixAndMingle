  ...
  try {
      ...
      try {
        ...
        if (bucketError) {
          if (bucketError.message.includes("does not exist")) {
            ...
            warnings.push(
              `⚠️ Bucket '${bucket}' issue: ${bucketError.message}`,
            );
        ...
        issues.push(
          `❌ Bucket '${bucket}' check failed: ${storageErr.message}`,
        );
    ...
    if (issues.length > 0) {
      ...
      if (issues.some((i) => i.includes("Table"))) {
        console.log(
          "   • Run database setup script (SETUP_DATABASE_DIRECT.sql)",
        );
      }
    ...
    const status =
      criticalIssues === 0 ? "PRODUCTION READY" : "NEEDS CRITICAL FIXES";
