  ...
  try {
    ...
    try {
      ...
      if (signupPageResponse.ok) {
        ...
        if (
          htmlContent.includes("signup") ||
          htmlContent.includes("register") ||
          htmlContent.includes("Create Account")
        ) {
        ...
        console.log(
          "❌ Signup page failed to load:",
          signupPageResponse.status,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (frontendSignupResponse.ok) {
        ...
        console.log(
          "   Response:",
          JSON.stringify(frontendSignupData, null, 2),
        );
      } else {
    ...
    try {
      ...
      console.log(
        "   Headers:",
        Object.fromEntries(corsTest.headers.entries()),
      );
    } catch (error) {
    ...
    for (const test of validationTests) {
      ...
      if (
        test.data.email &&
        test.data.password &&
        test.data.password === test.data.confirmPassword
      ) {
