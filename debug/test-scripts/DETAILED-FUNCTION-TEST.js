...
async function runAllFunctionTests() {
  log(
    `${colors.bold}🧪 DETAILED FUNCTION TEST SUITE${colors.reset}`,
    colors.blue,
  );
  ...
  if (passed >= total * 0.9) {
    log(
      "🎉 EXCELLENT: Your authentication system is fully functional!",
      colors.green,
    );
    ...
    log(
      "👍 GOOD: Most functions working, minor issues to resolve",
      colors.yellow,
    );
  ...
  if (
    results["Google OAuth Config"] &&
    !results["Google OAuth Status (POST)"]
  ) {
  ...
  if (
    results["Login Page Accessibility"] &&
    results["Signup Page Accessibility"]
  ) {
  ...
  const scoreColor =
    score >= 90 ? colors.green : score >= 75 ? colors.yellow : colors.red;
  log(
    `${score}% - ${score >= 90 ? "PRODUCTION READY" : score >= 75 ? "ALMOST READY" : "NEEDS WORK"}`,
    scoreColor,
  );
}
