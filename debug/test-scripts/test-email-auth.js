...
async function testEmailAuthSystem() {
  ...
  try {
    ...
    console.log("👤 Signup Test:", {
      ...
      email_service: data.email?.configured
        ? "✅ Configured"
        : "❌ Not configured",
    });
  ...
  try {
    ...
    if (data.reset_link) {
      console.log(
        "🔗 Direct reset link provided (email service not configured)",
      );
    }
  ...
  const envChecks = [
    ...
    { name: "NEXT_PUBLIC_APP_URL", required: true },
    {
      name: "RESEND_KEY",
      required: false,
      note: "Required for email delivery",
    },
  ];
  ...
  envChecks.forEach((check) => {
    ...
    console.log(
      `   • ${check.name}: ${check.required ? "[Required]" : "[Optional]"}${check.note ? ` - ${check.note}` : ""}`,
    );
  });
  ...
  console.log(
    "   • Test this script: node test-email-auth.js your-email@example.com",
  );
