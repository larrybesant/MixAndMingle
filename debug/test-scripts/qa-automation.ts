...
class QATestSuite {
    ...
    this.page.on("response", (response) => {
      if (!response.ok() && response.status() !== 404) {
        ...
        if (response.status() === 405) {
          this.logIssue(
            "405_error",
            `405 Method Not Allowed: ${response.url()}`,
          );
        }
      }
    });
  ...
  async testLandingPage() {
    ...
    try {
      ...
      await expect(this.page).toHaveTitle(/Mix.*Mingle/i);

      // Check for main elements
      const hasSignInButton = await this.page
        .locator("text=Sign In")
        .isVisible();
      const hasBrowseRooms = await this.page
    ...
    try {
      ...
      if (await emailField.isVisible()) {
        ...
        const hasError = await this.page
          .locator(".text-red-400, .bg-red-500")
          .isVisible();
        const hasSuccess = await this.page
          .locator(".text-green-400, .bg-green-500")
          .isVisible();
  ...
  async testPasswordResetFlow() {
    ...
    try {
      ...
      const emailField = this.page.locator('input[type=\"email\"]');
      const submitButton = this.page.locator(
        'button:has-text(\"Send Reset Link\"), button[type=\"submit\"]',
      );
      ...
      if (await emailField.isVisible()) {
        ...
        if (hasError) {
          ...
          if (errorText?.includes("405")) {
            this.logIssue(
              "405_error",
              "Password reset returning 405 error",
              "critical",
            );
          }
        }
    ...
    for (const page of pagesToTest) {
      try {
        ...
        if (!hasContent) {
          this.logIssue(
            "page_load_error",
            `${page.name} page failed to load content`,
            "medium",
          );
        }
        ...
        if (currentUrl.includes("/login") && !page.path.includes("/login")) {
          console.log(
            `ℹ️ ${page.name} requires authentication (redirected to login)`,
          );
        }
      } catch (error) {
        this.logIssue(
          "navigation_error",
          `Failed to navigate to ${page.name}: ${error}`,
          "medium",
        );
      }
    }
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.status() === 405) {
          ...
          this.logIssue(
            "api_error",
            `API error ${response.status()} on ${endpoint.path}`,
            "medium",
          );
        }
      } catch (error) {
        this.logIssue(
          "api_test_error",
          `Failed to test ${endpoint.path}: ${error}`,
          "low",
        );
      }
    }
