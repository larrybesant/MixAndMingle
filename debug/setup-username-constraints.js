...
async function addUsernameConstraints() {
  ...
  try {
    ...
    if (duplicateUsernames.length > 0) {
      console.log(
        "⚠️  Found duplicate usernames (case-insensitive):",
        duplicateUsernames,
      );
      console.log(
        "You may need to clean these up manually before adding constraints.",
      );
    } else {
    ...
    if (error) {
      ...
      if (constError) {
        ...
        console.log(
          "⚠️  Could not add constraint via RPC. Please run the SQL manually in Supabase.",
        );
