...
async function checkSupabaseConnection() {
  ...
  try {
    ...
    const supabase =
      window.supabase ||
      (await import("/utils/supabase/client")).createClient();
...
async function createTestAccount() {
  ...
  try {
    ...
    if (signUpError) {
      console.log(
        "⚠️ Signup error (might already exist):",
        signUpError.message,
      );
    } else {
...
function manualAuthOverride() {
  console.log(
    "🚨 MANUAL AUTH OVERRIDE - Use this to bypass auth for UI testing",
  );
...
async function runEmergencyAuthFix() {
  ...
  if (accountOk && emailOk) {
    ...
    console.log(
      "5. Check Redirect URLs include: http://localhost:3000/dashboard",
    );
