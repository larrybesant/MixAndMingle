  ...
  try {
    ...
    console.log(
      cleanedUp
        ...
        : `⚠️ ${debugFiles.length} debug files remain`,
    );
    if (!cleanedUp) {
    ...
    if (adminPageExists) {
      ...
      adminSecured =
        adminContent.includes("ADMIN_EMAIL") &&
        adminContent.includes("router.replace");
    }

    console.log(
      adminSecured
        ? "✅ Admin security: OK"
        : "⚠️ Admin security: Check needed",
    );
    ...
    checks.forEach((check) => {
      console.log(
        `${check.status ? "✅" : "❌"} ${check.name}: ${check.status ? "PASS" : "FAIL"}`,
      );
    });
    ...
    if (passedChecks === totalChecks) {
      ...
      console.log(
        "Core functionality works, address warnings before full launch",
      );
    } else {
