...
async function comprehensiveDiagnostic() {
  ...
  try {
    ...
    if (bucketsError) {
      ...
      if (buckets.length === 0) {
        issues.push(
          "⚠️ No buckets visible to anon user - this might be a permission issue",
        );
    ...
    if (issues.length > 0) {
      ...
      const warningIssues = issues.filter(
        (i) => i.includes("⚠️") && !i.includes("CRITICAL"),
      );
      ...
      if (criticalIssues.length > 0) {
        ...
        if (issues.some((i) => i.includes("does not exist"))) {
          console.log(
            "   1. Verify community-images bucket exists in Supabase dashboard",
          );
      ...
      if (warningIssues.length > 0) {
        ...
        if (issues.some((i) => i.includes("No buckets visible"))) {
          console.log(
            "   • This is normal - anon users may not list all buckets",
          );
        }
  ...
  console.log(
    "\n🎯 STATUS:",
    issues.filter((i) => i.includes("CRITICAL")).length === 0
