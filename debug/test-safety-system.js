  ...
  for (const endpoint of testEndpoints) {
    try {
      ...
      if (response.status === 401) {
        console.log(
          `   ✅ Endpoint ${endpoint} exists and has auth protection`,
        );
...
async function testAgeVerification() {
  ...
  const testCases = [
    { birthDate: "2010-01-01", expectedAge: 14, description: "Teen user" },
    {
      ...
      expectedAge: 9,
      description: "Under 13 (should be rejected)",
    },
    { birthDate: "1990-01-01", expectedAge: 34, description: "Adult user" },
    {
      birthDate: "2008-01-01",
      expectedAge: 16,
      description: "Minor requiring consent",
    },
  ];
  ...
  for (const testCase of testCases) {
    ...
    if (Math.abs(age - testCase.expectedAge) <= 1) {
      console.log(
        `   ✅ Age calculation correct for ${testCase.description}: ${age} years`,
      );
    } else {
      console.log(
        `   ❌ Age calculation incorrect for ${testCase.description}: got ${age}, expected ~${testCase.expectedAge}`,
      );
    }
  }
}
