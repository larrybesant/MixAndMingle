...
function checkRateLimit(ip: string, limit = 10, windowMs = 60_000): boolean {
  ...
  const globalWithRateLimit = global as typeof globalThis & { pushRateLimit?: RateLimitStore }
  if (!globalWithRateLimit.pushRateLimit) globalWithRateLimit.pushRateLimit = {}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return new NextResponse(JSON.stringify({ error: "Invalid subscription data" }), {
      status: 400,
