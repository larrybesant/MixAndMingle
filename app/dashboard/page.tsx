...
export default function DashboardPage() {
  ...
  const { conversations, loading: messagesLoading } = useRecentMessages(
    user?.id || null,
  );
  ...
  useEffect(() => {
    async function getUser() {
      ...
      const { data: profileData } = await supabase
        .from("profiles")
        .select(
          "id, full_name, username, avatar_url, bio, music_preferences, created_at, gender, relationship_style, bdsm_preferences, show_bdsm_public, is_dating_visible",
        )
        .eq("id", data.user.id)
        .single();
      if (profileData) {
        setProfile({
          ...
          gender:
            typeof profileData.gender === "string"
              ...
              : undefined,
          relationship_style:
            typeof profileData.relationship_style === "string"
              ? profileData.relationship_style
              : null,
          bdsm_preferences:
            typeof profileData.bdsm_preferences === "string"
              ? profileData.bdsm_preferences
              : null,
  ...
  function getProfileCompleteness(profile: DashboardProfile | null) {
    ...
    if (profile.music_preferences && profile.music_preferences.length > 0)
      complete++;
  ...
  if (profile) {
    ...
    if (!profile.music_preferences || profile.music_preferences.length === 0)
      missingFields.push("Music Preferences");
  ...
  if (loading)
    return <div className="text-white p-8 animate-pulse">Loading...</div>;
    ...
    <ErrorBoundary>
      <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-2 sm:px-0">
        ...
        >
          ...
          />
          <button
            className="bg-blue-600 px-4 py-2 rounded font-bold mt-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
            type="submit"
          >
            Update Profile
          </button>
          {formMessage && (
            <div
              className="text-sm text-white bg-black/60 rounded p-2 mt-2"
              aria-live="polite"
            >
              {formMessage}
            </div>
        ...
        <section className="mb-4 text-center text-sm text-gray-300">
          ...
          {profile && Array.isArray(profile.music_preferences)
            ? profile.music_preferences.join(", ")
            : profile?.music_preferences}
        </section>
        <section className="mb-4 text-center text-sm text-gray-300">
          <span className="font-semibold">Dating/Matchmaking:</span>{" "}
        ...
        {profile?.bdsm_preferences && (
          <section className="mb-4 text-center text-sm bg-gray-800/80 p-3 rounded-lg">
            <span className="font-semibold text-purple-300">
              BDSM / Kink / Other (private):
            </span>{" "}
            {profile.bdsm_preferences}
            {profile.show_bdsm_public && (
              <span className="ml-2 text-green-400">(Public)</span>
            )}
          </section>
        )}
        <section className="w-full max-w-md mb-6">
              ...
              {missingFields.map((field) => (
                <span key={field}>
                  ...
                  {field === "Username" ? (
                    <a
                      href="#username"
                      className="underline text-blue-400 ml-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                    >
                      Edit
                    </a>
                  ) : null}
          ...
          {completeness < 100 && (
            <Link
              href="/create-profile"
              className="block mt-2 bg-blue-700 text-white px-4 py-2 rounded font-bold text-center hover:bg-blue-800 transition focus:outline-none focus:ring-2 focus:ring-blue-400"
            >
              Complete Profile
            </Link>
          )}
        ...
        <nav className="flex gap-4 mb-8">
          ...
          <Link
            href="/settings"
            className="bg-gray-700 text-white px-4 py-2 rounded font-bold hover:bg-gray-800 transition focus:outline-none focus:ring-2 focus:ring-gray-400"
          >
            Settings
          </Link>
        </nav>
        ...
        <section className="w-full max-w-md mb-8">
          ...
          {friendsLoading ? (
            ...
            <div className="bg-gray-800 rounded-lg p-4 min-h-[60px] text-gray-300 italic">
                ...
                (msg: {
                  ...
                  <li key={msg.id} className="flex items-center gap-3 py-2">
                    ...
                    <span>
                      ...
                      {(msg.content ? msg.content.length : msg.message.length) >
                      40
                        ? "..."
                        : ""}
                    </span>
