  ...
  useEffect(() => {
    async function fetchUserAndFriends() {
      try {
        ...
        const parsedRequests = FriendRequestListSchema.safeParse(
          requestsData || [],
        );
      ...
      {loading ? (
        ...
        <>
              ...
              <ul className="bg-gray-800 rounded-lg p-4 divide-y divide-gray-700">
                {requests.map((r) => (
                  <li key={r.id} className="flex items-center gap-3 py-2">
                    <img
                      src={r.profiles?.avatar_url || "/file.svg"}
                      alt="avatar"
                      className="w-8 h-8 rounded-full bg-gray-600"
                    />
                    <span>{r.profiles?.username || "Unknown"}</span>
                    <button
                      className="ml-auto bg-blue-600 px-3 py-1 rounded font-bold"
          ...
          <section className="w-full max-w-md mb-8">
            ...
            {friends.length === 0 ? (
              <div className="text-gray-400 italic">No friends yet.</div>
            ) : (
              <ul className="bg-gray-800 rounded-lg p-4 divide-y divide-gray-700">
                {friends.map((f) => (
                  <li key={f.id} className="flex items-center gap-3 py-2">
                    <img
                      src={f.profiles?.avatar_url || "/file.svg"}
                      alt="avatar"
                      className="w-8 h-8 rounded-full bg-gray-600"
                    />
                    <span>{f.profiles?.username || "Unknown"}</span>
                    <button
                      className="ml-auto bg-red-600 px-3 py-1 rounded font-bold"
