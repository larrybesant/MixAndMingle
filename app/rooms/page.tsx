    ...
    async function fetchRooms() {
      ...
      setRooms(
        (data || []).map((room) => ({
          ...
          genre: String(room.genre),
          is_live: Boolean(room.is_live),
          viewer_count: Number(room.viewer_count),
          description:
            typeof room.description === "string" ? room.description : "",
          host_id: typeof room.host_id === "string" ? room.host_id : "",
          tags: Array.isArray(room.tags) ? room.tags.map(String) : [],
          created_at:
            typeof room.created_at === "string" ? room.created_at : "",
        })),
      );
      setLoading(false);
    }
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {rooms.map((room) => (
            <div
              key={room.id}
              className="bg-gray-800 rounded-2xl p-6 shadow-lg flex flex-col items-center w-full max-w-xs mx-auto"
            >
