    ...
    async function fetchUserAndMessages() {
      try {
        ...
        const { data: messagesData, error: msgError } = await supabase
          .from("messages")
          .select(
            "id, message, created_at, sender:sender_id(username, avatar_url), receiver:receiver_id(username, avatar_url)",
          )
          .or(
  ...
  return (
    <ErrorBoundary>
      <main className="min-h-screen bg-black text-white flex flex-col items-center px-2 py-8">
        ...
        <form
          onSubmit={handleSend}
          className="flex flex-col md:flex-row gap-2 mb-6 w-full max-w-xs sm:max-w-2xl mx-auto"
        >
          <input
        ...
        {sendStatus && (
          <div className="text-sm text-white bg-black/60 rounded p-2 mb-4">
            {sendStatus}
          </div>
        )}
      </main>
