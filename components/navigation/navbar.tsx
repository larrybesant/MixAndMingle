...
export const Navbar = () => {
  ...
  return (
    <nav
      className={cn(
        "w-full flex items-center justify-between px-6 py-4 bg-stone-900 text-white",
      )}
    >
          ...
          {showDropdown && (
            <div className="absolute right-0 mt-2 w-72 bg-white text-black rounded shadow-lg z-50 max-h-96 overflow-y-auto">
              ...
              {notifications.map((n, i) => (
                <div
                  key={i}
                  className={`p-2 border-b last:border-b-0 ${!n.read ? "bg-blue-100" : ""}`}
                >
                  ...
                  {n.type === "room_invite" && n.data?.room_id ? (
                    <Link
                      href={`/room/${String(n.data.room_id)}`}
