...
if (fs.existsSync(signupPagePath)) {
  ...
  const hasProfileCreation = content.includes('profiles') && content.includes('insert');
  console.log(`   ✅ Profile creation logic: ${hasProfileCreation ? 'PRESENT' : 'MISSING'}`);
  
  // Check for proper error handling
  const hasErrorHandling = content.includes('try') && content.includes('catch') && content.includes('setError');
  ...
  console.log(`   ${duplicateHandlers ? '❌' : '✅'} Duplicate handlers: ${duplicateHandlers ? 'FOUND (THIS CAUSES ISSUES)' : 'NOT FOUND'}`);
  
  // Check for loading states
  const hasLoadingState = content.includes('loading') && content.includes('setLoading');
  console.log(`   ✅ Loading states: ${hasLoadingState ? 'PRESENT' : 'MISSING'}`);
  
  // Check for validation
  const hasValidation = content.includes('isValidUsername') && content.includes('isValidEmail');
...
if (fs.existsSync(schemaPath)) {
  ...
  const hasProfilesTable = schema.includes('CREATE TABLE IF NOT EXISTS profiles');
  console.log(`   ✅ Profiles table: ${hasProfilesTable ? 'DEFINED' : 'MISSING'}`);
  
  const hasUsernameField = schema.includes('username TEXT UNIQUE');
  console.log(`   ✅ Username field: ${hasUsernameField ? 'DEFINED' : 'MISSING'}`);
  
  const hasAuthReference = schema.includes('REFERENCES auth.users(id)');
  console.log(`   ✅ Auth reference: ${hasAuthReference ? 'DEFINED' : 'MISSING'}`);
} else {
...
console.log(`   ✅ Environment files found: ${envFiles.length} (${envFiles.join(', ')})`);

if (envFiles.length === 0) {
  console.log('   ❌ No environment files found! Supabase won\'t work without .env.local');
}
...
console.log('   • Environment variables not set (SUPABASE_URL, SUPABASE_ANON_KEY)');
