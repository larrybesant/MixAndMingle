...
class QATestSuite {
  ...
  logIssue(type: string, message: string, severity: 'low' | 'medium' | 'high' | 'critical' = 'medium') {
    const issue = {
      type,
      message,
      severity,
  ...
  async testLandingPage() {
    ...
    try {
      ...
      if (!hasSignInButton) this.logIssue('missing_element', 'Sign In button not found', 'medium');
      if (!hasBrowseRooms) this.logIssue('missing_element', 'Browse Rooms button not found', 'medium');
      if (!hasGoLive) this.logIssue('missing_element', 'Go Live button not found', 'medium');
