...
async function generateAnalyticsReport() {
  ...
  try {
    ...
    if (!profilesError && profiles) {
      ...
      const recentUsers = profiles.filter(p => new Date(p.created_at) > yesterday);
      console.log(`🆕 New Users (24h): ${recentUsers.length}`);
      
      // Users created in last 7 days
      const weekAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      const weeklyUsers = profiles.filter(p => new Date(p.created_at) > weekAgo);
    ...
    if (!communitiesError && communities) {
      ...
      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
      const recentCommunities = communities.filter(c => new Date(c.created_at) > yesterday);
      ...
      const topCreators = Object.entries(creatorCounts)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 3);
      console.log(`🏆 Top Creators: ${topCreators.map(([id, count]) => `${count} communities`).join(', ')}`);
    }
    ...
    if (!membershipsError && memberships) {
      ...
      memberships.forEach(m => {
        membershipsByComm[m.community_id] = (membershipsByComm[m.community_id] || 0) + 1;
      });
      const avgMembers = Object.values(membershipsByComm).reduce((a, b) => a + b, 0) / Object.keys(membershipsByComm).length;
      ...
      const topCommunities = Object.entries(membershipsByComm)
        .sort(([,a], [,b]) => b - a)
        .slice(0, 5);
      console.log(`🔥 Top Communities: ${topCommunities.map(([id, count]) => `${count} members`).join(', ')}`);
    }
    ...
    if (!postsError && posts) {
      ...
      const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000);
      const recentPosts = posts.filter(p => new Date(p.created_at) > yesterday);
      console.log(`🆕 New Posts (24h): ${recentPosts.length}`);
      
      // Average posts per community
      const postsByCommunity = {};
      posts.forEach(p => {
        postsByCommunity[p.community_id] = (postsByCommunity[p.community_id] || 0) + 1;
      });
      const avgPosts = Object.values(postsByCommunity).reduce((a, b) => a + b, 0) / Object.keys(postsByCommunity).length;
    ...
    const communitiesPerUser = totalUsers > 0 ? (totalCommunities / totalUsers) : 0;
    const postsPerUser = totalUsers > 0 ? (totalPosts / totalUsers) : 0;
    const membershipsPerUser = totalUsers > 0 ? (totalMemberships / totalUsers) : 0;
    ...
    const engagementScore = Math.min(100, (communitiesPerUser * 20 + postsPerUser * 15 + membershipsPerUser * 10) * 10);
