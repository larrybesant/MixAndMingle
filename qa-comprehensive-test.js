...
class QATestSuite {
  ...
  async testCommunitiesSchema() {
    await this.test('Communities Database Schema', async () => {
      ...
      if (error && error.message.includes('relation "communities" does not exist')) {
        throw new Error('Communities schema not set up - visit /admin to set up');
      }
  ...
  async testCommunitiesFunctionality() {
    await this.test('Communities Page Functionality', async () => {
      ...
      const createButton = await this.page.$('[data-testid="create-community"], button:contains("Create Community")');
      ...
      if (!createButton && !searchInput) {
        ...
        if (!currentUrl.includes('/login') && !currentUrl.includes('/signup')) {
          throw new Error('Communities page elements not found and not redirected to auth');
        }
      }
    });
  }
  ...
  async runAllTests() {
    ...
    console.log('\n🎯 QA RECOMMENDATIONS:');
    if (this.testResults.failed === 0) {
      console.log('🎉 All tests passed! Application is ready for production deployment.');
    } else if (this.testResults.failed <= 2) {
      console.log('⚠️ Minor issues detected. Address failed tests before deployment.');
    } else {
      console.log('🚨 Major issues detected. Requires immediate attention before deployment.');
    }
